...
import { useOrbis, OrbisProvider } from "@orbisclub/components";
import Postbox from "../Postbox";
import Post from "../Post";
import { LoadingCircle, Logo, EmptyStateComments } from "../../Icons";
...
const CommentsContent = ({ characterLimit, master }) => {
	...
	async function loadPosts() {
		...
		const commenters = data.map((comment = {}) => {
			const did = comment?.creator_details?.did;
			const username = comment?.creator_details?.profile?.username;
			return { did, username };
		});
		verifyUsernames(commenters);
...
function LoopComments({ comments, characterLimit, master }) {
	return comments.map((comment, key) => {
		if (
			comment.content.reply_to == master ||
			((!comment.content.reply_to || comment.content.reply_to == "") &&
				(!comment.content.master || comment.content.master == ""))
		) {
			return (
				<Comment
					comments={comments}
