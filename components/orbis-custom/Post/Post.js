...
export default function Post({
	...
	async function like(type) {
		...
		if (!hasAccess) {
			alert(
				"You need the required credentials to react to posts in this feed."
			);
			return;
		}
				...
				<div className={styles.replyToContainer}>
					<div style={{ display: "flex", marginRight: 10 }}>
						<div
							...
							style={{ borderColor: getThemeValue("border", theme, "main") }}
						></div>
						<UserPfp
							...
							>
								...
								<div className={styles.hideMobile}>
									...
									<a
										...
										href={
											"https://cerscan.com/mainnet/stream/" + post.stream_id
										}
										rel="noreferrer"
										target="_blank"
									>
		...
		return (
			<div
				...
				dangerouslySetInnerHTML={{
					__html: marked.parse(
						charLimit ? body?.substring(0, charLimit) + "..." : body
					),
				}}
			></div>
		);
				...
				<>
					{charLimit && post.content?.body?.length > charLimit ? (
						...
						<>
							...
							{post.indexing_metadata?.urlMetadata &&
								post.creator_details?.a_r > 15 && (
		...
		>
			...
			>
				...
				{metadata.description && (
					...
					>
						{metadata.description.length > 155 ? (
							<>{metadata.description}...</>
						) : (
							<>{metadata.description}</>
						)}
	...
	function DeleteButton() {
		switch (deletingStatus) {
			case 0:
				return (
					<div
						className="text-red-700 hover:bg-gray-50 flex items-center px-3 py-2 text-sm font-medium rounded-md cursor-pointer"
			...
			case 1:
				return (
					<div className="text-red-700 flex items-center px-3 py-2 text-sm font-medium rounded-md">
			...
			case 2:
				return (
					<div className="text-green-700 flex items-center px-3 py-2 text-sm font-medium rounded-md">
						<span className="truncate mr-2">Deleted</span>
					</div>
				);
		}
	}
...
let sendStyleReply =
	"inline-flex items-center rounded-full border border-transparent bg-[#4E75F6] px-4 py-1 text-sm font-medium text-white shadow-sm hover:bg-[#3E67F0] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 cursor-pointer";
