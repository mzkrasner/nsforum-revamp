...
import { useOrbis, AccessRulesModal } from "@orbisclub/components";
...
import { LoadingCircle, LockIcon, UnlockIcon, SendIcon } from "../../Icons";
...
export default function Postbox({
	...
	const { user, setUser, orbis, theme, context, accessRules, hasAccess } =
		useOrbis();
	...
	function addMention(mention) {
		...
		var _mentionTag =
			"<span style='color: " +
			getThemeValue("color", theme, "active") +
			"; font-weight: " +
			500 +
			";' class='mention' contentEditable='false' data-did='" +
			mention.did +
			"'>@" +
			_mentionName +
			"</span>&nbsp;";
		...
		return (
			<div className={styles.postboxGlobalContainer}>
				...
				<div className={styles.postboxContainer}>
					...
					>
						<div
							...
							style={{
								...
								backgroundColor: getThemeValue("input", theme, sharing)
									.background,
							}}
						>
							{/** Show reply if any */}
							{reply && (
								<div className={styles.postboxReplyContainer}>
									...
									>
										<Username details={reply.creator_details} />
									</div>
								</div>
							)}
					...
					{/** Show mentions box */}
					{mentionsBoxVis && <MentionsBox add={addMention} />}
				</div>
...
export const AccessRulesDetails = ({
	...
	return (
		<div
			...
			style={{ color: getThemeValue("color", theme, "secondary"), ...style }}
		>
			{user && hasAccess ? (
				<UnlockIcon
					style={{
