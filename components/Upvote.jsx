...
import NoAttestationModal from './NoAttestationModal';
...
export default function Upvote({
  ...
  const [showNoAttestationModal, setShowNoAttestationModal] = useState(false);

  const { hasAccess, fetching } = useContextAccess({
    context: global.orbis_context,
  });

  // only allow users with attestation to react
  const guardReaction = (fn) => {
    if (hasAccess) return () => fn();
    return () => setShowNoAttestationModal(true);
  };

  return (
    <>
      {showNoAttestationModal && (
        <NoAttestationModal
          closeModal={() => setShowNoAttestationModal(false)}
        />
      )}
      <div
        className={clsx('h-fit rounded-md border p-4 py-2 mr-4 flex flex-col', {
